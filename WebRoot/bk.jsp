...
<script  language="javascript" >
    function top2(){
        form3.action="<%=basePath%>bk.jsp?page=1";
        form3.submit();
    }
    function last2(){
        if(form3.pageCount.value==0){//如果总页数为0，那么最后一页为1，也就是第一页，而不是第0页
            ...
            form3.action="<%=basePath%>bk.jsp?page="+form3.pageCount.value;
            form3.submit();
        }
    }
    function pre2(){
        ...
        if(page<=1){
            ...
            form3.action="<%=basePath%>bk.jsp?page="+(page-1);
            form3.submit();
        }
    }
    ...
    function bjump2(){
        ...
        if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
            if(pageCount==0){
                ...
                form3.action="<%=basePath%>bk.jsp?page="+pageCount;
                form3.submit();
            }
        }
            ...
            if(page==0){
                ...
                form3.action="<%=basePath%>bk.jsp?page="+page;
                form3.submit();
            }else{
                form3.action="<%=basePath%>bk.jsp?page="+page;
                form3.submit();
                    ...
                    <div id="gallery-container">
                        <div class="row gallery-3-columns isotope-x clearfix">
                            <%
                                ...
                                int cou = cb.getMessageCount("select count(*) from bk ");//得到信息总数
                                ...
                                List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from bk order by id desc",6);
                            ...
                            <div class="gallery-item isotope-item print-media col-lg-4 col-sm-4 col-xs-6">
                                <div class="inner-contents">
                                    <figure>
                                        <a class="gallery-btn swipebox" href="<%=basePath+pagelist2.get(3).toString()%>">查看更多</a>
                                        <div class="media-container"></div>
                                        <img src="<%=basePath+pagelist2.get(3).toString()%>" width="310" height="235"/> </figure>
                                    <h5 class="item-title"><a href="<%=basePath%>bkinfo.jsp?id=<%=pagelist2.get(0).toString()%>"><%=pagelist2.get(1).toString()%></a></h5>
                                </div>
                            </div>
                            <%}} %>
                        </div>
                    ...
                    <div class="tabbed custom-tabbed">
                        <div class="block current">
                            <ul class="widget-list">
                                ...
                                <li>
                                    <figure>
                                        <a href="<%=basePath%>xwinfo.jsp?id=<%=list2.get(0).toString()%>"><img src="<%=basePath+list2.get(3).toString()%>" /></a>
                                    </figure>
