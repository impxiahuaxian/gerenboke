...
<script  language="javascript" >
    function top2(){
        form3.action="<%=basePath%>member/fw/sinfo.jsp?page=1";
        form3.submit();
    }
    function last2(){
        if(form3.pageCount.value==0){//如果总页数为0，那么最后一页为1，也就是第一页，而不是第0页
            ...
            form3.action="<%=basePath%>member/fw/sinfo.jsp?page="+form3.pageCount.value;
            form3.submit();
        }
    }
    function pre2(){
        ...
        if(page<=1){
            ...
            form3.action="<%=basePath%>member/fw/sinfo.jsp?page="+(page-1);
            form3.submit();
        }
    }
    ...
    function bjump2(){
        ...
        if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
            if(pageCount==0){
                ...
                form3.action="<%=basePath%>member/fw/sinfo.jsp?page="+pageCount;
                form3.submit();
            }
        }
            ...
            if(page==0){
                ...
                form3.action="<%=basePath%>member/fw/sinfo.jsp?page="+page;
                form3.submit();
            }else{
                form3.action="<%=basePath%>member/fw/sinfo.jsp?page="+page;
                form3.submit();
...
<div class="x-body">
    <!--查询-->
    <form action="<%=basePath%>member/fw/sinfo.jsp" method="post" style="width:100%">
        <div class="layui-form-pane">
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline" style="width:200px">
                    <select name="hx" style="width:200px; height:35px;border:1px solid #e6e6e6;" required>
                        <option value="">请选择</option>
    ...
    <div class="tools">
        <ul class="toolbar">
            ...
            <li><a href="<%=basePath%>member/fw/add.jsp?method=addfw"><span><img src="<%=basePath%>admin/statics/images/t01.png" /></span>添加</a></li>
        </ul>
    </div>
    <!--查询-->
    <!--列表-->
    <form action="<%=basePath%>fw/batchDel" method="post" name="form1">
        <table class="tablelist"  border="1">
            ...
            <%
                ...
                int cou = cb.getMessageCount("select count(*) from fw where bt like '%"+bt+"%' and fl='"+fl+"' and hx='"+hx+"' and member='"+member+"' ");//得到信息总数
                ...
                List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from fw where bt like '%"+bt+"%' and fl='"+fl+"' and hx='"+hx+"' and member='"+member+"' order by id desc",11);
            ...
            <tr>
                ...
                <td><%=i+1 %></td>
                <td><a href="<%=basePath%>fwinfo.jsp?id=<%=pagelist2.get(0).toString()%>" target="_blank"><%=pagelist2.get(1).toString() %></a></td>
                <td><%=pagelist2.get(2).toString() %></td>
                <td><%=pagelist2.get(3).toString() %></td>
                <td><%=pagelist2.get(4).toString() %></td>
                <td><%=pagelist2.get(5).toString() %></td>
                <td><img src="<%=basePath+pagelist2.get(6).toString()%>" width="100" height="100"></td>
                <td><%=pagelist2.get(8).toString() %></td>
                <td><%=pagelist2.get(9).toString() %></td>
                <td><%=pagelist2.get(10).toString() %></td>
                <td>
                    <a href="<%=basePath%>member/fw/add.jsp?method=upfw&id=<%=pagelist2.get(0).toString()%>">修改</a>
                    <a onclick="if (confirm('确定要删除吗？')) return true; else return false;" href="<%=basePath%>fw/del/<%=pagelist2.get(0).toString()%>">删除</a>
                </td>
            </tr>
                    <%}}%>
                </table>
    ...
    <div class="page" v-show="show">
        <div class="pagelist">
            <form action="" method="post" name="form3">
                ...
                <input type="hidden" name="hx" value="<%= hx%>" />
