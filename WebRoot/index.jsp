...
<section class="services-section" id="index-cate">
    <div class="container">
        <div class="services-box">
            <div class="row">
                <%
                    List fw2list=cb.getCom("select * from bk order by id desc limit 4",6);
                    if(!fw2list.isEmpty()){
                ...
                <div class="col-md-3 col-sm-6 col-xs-6 cate-itme">
                    <div class="services-post">
                        <a href="<%=basePath%>bkinfo.jsp?id=<%=list2.get(0).toString()%>" class="thumb-link"><img src="<%=basePath+list2.get(3).toString()%>" width="318" height="216"/></a>
                        <div class="services-content">
                            <h2><a href="<%=basePath%>bkinfo.jsp?id=<%=list2.get(0).toString()%>"><%=list2.get(1).toString()%></a></h2>
                            <p><%=list2.get(5).toString()%></p>
                            <a href="<%=basePath%>bkinfo.jsp?id=<%=list2.get(0).toString()%>" class="readmore">查看更多 <i class="fa fa-angle-double-right"></i></a>
                        </div>
                    </div>
                </div>
                <%}}%>
...
<section class="portfolio-section" id="index-portfolio">
    <div class="container">
        <h3 class="text-center section-title"><a href="javascript:;">热点博客</a> </h3>
        <div class="portfolio-box owl-wrapper">
            <div class="owl-carousel" data-num="4">
                <%
                    List bklist=cb.getCom("select * from bk order by dj desc limit 8",6);
                    if(!bklist.isEmpty()){
                        for(int i=0;i<bklist.size();i++){
                            List list2 =(ArrayList)bklist.get(i);
                %>
                <div class="item project-post">
                    <div class="project-gallery"> <img src="<%=basePath+list2.get(3).toString()%>"  width="318" height="216"/>
                        <div class="hover-box">
                            <div class="inner-hover">
                                <h2><a href="<%=basePath%>bkinfo.jsp?id=<%=list2.get(0).toString()%>"><%=list2.get(1).toString()%></a></h2>
                                <span><%=list2.get(5).toString()%> </span> </div>
                        </div>
                    </div>
                    <h3 class="iport-h3-title"><a href="<%=basePath%>bkinfo.jsp?id=<%=list2.get(0).toString()%>"><%=list2.get(1).toString()%></a></h3>
                </div>
                <%}}%>
            </div>
        </div>
            ...
            <div class="owl-carousel" data-num="4">
                ...
                <div class="item news-post">
                    <div class="news-gallery">
                        <a href="<%=basePath%>xwinfo.jsp?id=<%=list2.get(0).toString()%>" class="thumb-link"><img src="<%=basePath+list2.get(4).toString()%>"  width="318" height="216"/></a>
                        <div class="date-post">
